...
namespace FzLib.Basic
{
    ...
    public class diff_match_patch
    {
        ...
        public int diff_commonPrefix(string text1, string text2)
        {
            ...
            int n = SMath.Min(text1.Length, text2.Length);
            for (int i = 0; i < n; i++)
            {
                if (text1[i] != text2[i])
                {
        ...
        protected int diff_commonOverlap(string text1, string text2)
        {
            ...
            int text_length = SMath.Min(text1_length, text2_length);
            ...
            while (pointer < diffs.Count)
            {
                ...
                {  // an insertion or deletion
                    ...
                    if (lastEquality != null && (lastEquality.Length
                        <= SMath.Max(length_insertions1, length_deletions1))
                        && (lastEquality.Length
                            <= SMath.Max(length_insertions2, length_deletions2)))
                    {
                        // Duplicate record.
        ...
        public int match_main(string text, string pattern, int loc)
        {
            ...
            loc = SMath.Max(0, SMath.Min(loc, text.Length));
            if (text == pattern)
            {
        ...
        protected int match_bitap(string text, string pattern, int loc)
        {
            ...
            if (best_loc != -1)
            {
                score_threshold = SMath.Min(match_bitapScore(0, best_loc, loc,
                    pattern), score_threshold);
                // What about in the other direction? (speedup)
                best_loc = text.LastIndexOf(pattern,
                    SMath.Min(loc + pattern.Length, text.Length),
                    StringComparison.Ordinal);
                if (best_loc != -1)
                {
                    score_threshold = SMath.Min(match_bitapScore(0, best_loc, loc,
            ...
            for (int d = 0; d < pattern.Length; d++)
            {
                ...
                bin_max = bin_mid;
                int start = SMath.Max(1, loc - bin_mid + 1);
                int finish = SMath.Min(loc + bin_mid, text.Length) + pattern.Length;
                    ...
                    if ((rd[j] & matchmask) != 0)
                    {
                        ...
                        if (score <= score_threshold)
                        {
                            ...
                            if (best_loc > loc)
                            {
                                ...
                                start = SMath.Max(1, 2 * loc - best_loc);
                            }
                            else
                            {
        ...
        protected void patch_addContext(Patch patch, string text)
        {
                ...
                && pattern.Length < Match_MaxBits - Patch_Margin - Patch_Margin)
            {
                ...
                pattern = text.JavaSubstring(SMath.Max(0, patch.start2 - padding),
                    SMath.Min(text.Length, patch.start2 + patch.length1 + padding));
            }
            // Add one chunk for good luck.
            padding += Patch_Margin;

            // Add the prefix.
            string prefix = text.JavaSubstring(SMath.Max(0, patch.start2 - padding),
              patch.start2);
            if (prefix.Length != 0)
            {
                ...
                else
                {
                    ...
                    if (end_loc == -1)
                    {
                        text2 = text.JavaSubstring(start_loc,
                            SMath.Min(start_loc + text1.Length, text.Length));
                    }
                    else
                    {
                        text2 = text.JavaSubstring(start_loc,
                            SMath.Min(end_loc + this.Match_MaxBits, text.Length));
                    }
                    if (text1 == text2)
                    {
                ...
                while (bigpatch.diffs.Count != 0)
                {
                        ...
                        && patch.length1 < patch_size - this.Patch_Margin)
                    {
                        ...
                        else
                        {
                            ...
                            diff_text = diff_text.Substring(0, SMath.Min(diff_text.Length,
                    ...
                    precontext = precontext.Substring(SMath.Max(0,
