...
namespace FzLib.Geography.IO.Tile
{
   ...
   */
         ...
         * 从经纬度获取某一级别瓦片坐标编号
         */
        public static (int x, int y) GeoPointToTile(Point p, int level)
        {
            return GeoPointToTile(p.Y, p.X, level);
        }
         ...
         */
        public static (int x, int y) GeoPointToPixel(Point p, int level)
        {
            return GeoPointToPixel(p.Y, p.X, level);
        }
