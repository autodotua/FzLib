   ...
   public class DistanceAnalysis
    {
        ...
        public static double GetDistance(double lat1, double lng1, double lat2 ,double lng2)
        {
            ...
            double result = 2 * Math.Asin(Math.Sqrt(Math.Pow(Math.Sin(a / 2), 2) + Math.Cos(radLat1) * Math.Cos(radLat2) * Math.Pow(Math.Sin(b / 2), 2))) *Constant. EarthRadius;
            return result;
        }
        public static double GetDistance(GeoPoint p1,GeoPoint p2)
        {
            double radLat1 = Rad(p1.Latitude);
            double radLng1 = Rad(p1.Longitude);
            double radLat2 = Rad(p2.Latitude);
            double radLng2 = Rad(p2.Longitude);
            double a = radLat1 - radLat2;
            double b = radLng1 - radLng2;
            double result = 2 * Math.Asin(Math.Sqrt(Math.Pow(Math.Sin(a / 2), 2) + Math.Cos(radLat1) * Math.Cos(radLat2) * Math.Pow(Math.Sin(b / 2), 2))) * Constant.EarthRadius;
            return result;
