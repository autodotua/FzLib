    ...
    private async void DialogButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        switch ((sender as Button).Tag as string)
        {
            ...
            case "14":
                ...
                int? index = await this.ShowSelectItemDialog("标题", items, "提示消息", "额外按钮",
                    async () => await this.ShowOkDialogAsync("单击了额外按钮"));
            ...
            case "15":
                ...
                VM.Message = result
                    ? VM.Message = $"选择了{string.Join('，', checkItems.Where(p => p.IsChecked).Select(p => p.Title))}"
                    : "取消了选择";
                break;
