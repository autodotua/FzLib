...
public partial class MainWindow : Window
{
    ...
    private async void DialogButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        else
        {
            ...
            {
                case "1":
                    await g.ShowOkDialogAsync("标题", "信息正文");
                    break;
                case "2":
                    await g.ShowOkDialogAsync("标题", "信息正文", string.Concat(Enumerable.Repeat("详细内容", 1000)));
                    break;
                case "3":
                    await g.ShowWarningDialogAsync("标题", "警告正文");
                    break;
                case "4":
                    await g.ShowErrorDialogAsync("标题", "错误正文");
                    break;
                case "5":
                    try
                    {
                    ...
                    {
                        while (await g.ShowErrorDialogAsync("错误信息", ex, true))
                        {
                    ...
                    break;
                case "6":
                    VM.Message = (await g.ShowYesNoDialogAsync("标题", "询问内容")).Value ? "单击“是”" : "单击“否”";
                    break;
                case "7":
                    switch (await g.ShowYesNoDialogAsync("标题", "询问内容", cancelButon: true))
                    {
                        case true:
                            VM.Message = "单击“是”";
                            break;
                ...
                case "8":
                    VM.Message = "输入内容：" + await g.ShowInputTextDialogAsync("标题", "请输入：", "默认值", "水印");
                    break;

                case "9":
                    VM.Message = "输入内容：" + await g.ShowInputTextDialogAsync("标题", "必须长度>5且不能出现数字：", "默认值", "水印", text =>
                    {
                        if (text.Length <= 5)
                        {
                            throw new ArgumentException("长度必须>5");
                ...
                case "10":
                    VM.Message = "输入内容：" + await g.ShowInputPasswordDialogAsync("标题", "请输入密码：", "水印");
                    break;

                case "11":
                    VM.Message = "输入内容：" + await g.ShowInputMultiLinesTextDialogAsync("标题", "请输入多行文本：");
                    break;

                case "12":
                    VM.Message = "输入内容：" + await g.ShowInputNumberDialogAsync<double>("标题", "请输入数字：");
                    break;

                case "13":
                    VM.Message = "输入内容：" + await g.ShowInputNumberDialogAsync<int>("标题", "请输入整数：");
                    break;
                ...
                case "14":
                    SelectDialogItem[] items = [
                        ...
                        new SelectDialogItem("第二条", "详情"),
                        new SelectDialogItem("第三条"),
                        new SelectDialogItem("第四条", "单击直接触发", async () => await g.ShowOkDialogAsync("单击了第四条")),
                    ];
                    int? index = await g.ShowSelectItemDialog("标题", items, "提示消息", "额外按钮", async () => await g.ShowOkDialogAsync("单击了额外按钮"));
                    VM.Message = index.HasValue ? $"单击了{items[index.Value].Title}" : "没有选择";
                    break;
                ...
                case "15":
                    ...
                    bool result = await g.ShowCheckItemDialog("标题", checkItems, "需要选择2-4个", 2, 4);
                    VM.Message = result ? VM.Message = $"选择了{string.Join('，', checkItems.Where(p => p.IsChecked).Select(p => p.Title))}" : "取消了选择";
                    break;
            }
        }
